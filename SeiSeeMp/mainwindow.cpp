...
{
    {
        ...
        QAction *actionLoad_Navigation_File = new QAction(tr("Load Navigation File"), this);
        connect(actionLoad_Navigation_File, &QAction::triggered, this, &MainWindow::loadNavigationFile);
        menuBar()->addAction(actionLoad_Navigation_File);
...
void MainWindow::on_pushButton_clicked()
{
    ...
    QString fileName = QFileDialog::getOpenFileName(this, tr("Open File"), "", tr("Data Files (*.dat);;All Files (*)"));
    ...
    if (!fileName.isEmpty()) {
        ...
        // Read the file and create a model
        // Read the file and create a model
        QTextStream in(&file);
        ...
        while (!in.atEnd()) {
            ...
            if (line.startsWith("G")) {
                ...
                double x = coords[3].toDouble();
                double y = coords[4].toDouble();
...
void MainWindow::loadNavigationFile()
{
    QString fileName = QFileDialog::getOpenFileName(this, tr("Open Navigation File"), QString(), tr("Data Files (*.dat)"));
    ...
    if (!fileName.isEmpty()) {
        ...
        if (file.open(QIODevice::ReadOnly)) {
            ...
            QLabel *label = new QLabel(QString::fromUtf8(contents));
...
{
    lne dialog(this); // Create a new instance of the lne dialog
       dialog.exec(); // Show the dialog
}


void MainWindow::on_actionLoad_Navigation_File_triggered()
{
    // Open a file dialog to select the navigation file
       QString filename = QFileDialog::getOpenFileName(this, tr("Open Navigation File"), "", tr("Navigation Files (*.dat)"));
       ...
       if (file.open(QIODevice::ReadOnly)) {
           ...
           // Set the text of the QTextEdit in the UI to the file content
           QTextStream in(&fileContent);
          ui.textEdit_navigation->setPlainText(in.readAll());
          ui.textEdit_navigation->setReadOnly(true);
       }

       // Show the dialog and wait for the user to close it
       dialog.exec();

       // Update the content of the QFrame with the selected navigation file
       ...
       // Create a QVBoxLayout and set it as the layout for the QFrame
       QVBoxLayout *layout = new QVBoxLayout(m_frame);
       layout->setContentsMargins(0, 0, 0, 0);
       layout->addWidget(ui.textEdit_navigation);
}
